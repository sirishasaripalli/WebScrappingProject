...
public class LCHF2 extends hooksForScrapping {
    private ArrayList<String> eliminateList;
    
    private ArrayList<String> requiredIngredientsList;

    @Test
    public void extractRecipe() throws InterruptedException, IOException {
        eliminateList = ReadElliminationList.readColumnFromExcel("/Users/parikshit/Downloads/LCHFEllimanator (2).xlsx", 0);
        
        
        requiredIngredientsList = new ArrayList<>(Arrays.asList(
            "Fish", "prawn","poultry","egg","Onion","Garlic","turmeric","Ginger","Butter","ghee","hard cheese","paneer","cottage cheese ","sour cream ",
            "greek yogurt","hung curd","almond","pistachio","brazil nut","walnut","pine nut","hazelnut","pecan","hemp seed","sunflower seed","sesame seed",
        ...
        if (0 != lastPage) {
			for (int j = 1; j <= lastPage; j++) {
				...
				tlDriver.navigate().to("https://www.tarladalal.com/recipes-for-low-carb-veg-indian-recipes-757"
                    ...
                    } else if (!containsRequiredIngredients(requiredIngredientsList)) {
                        
                        System.out.println("Recipe does not contain required ingredients.");
                    } else {
    ...
    private boolean containsRequiredIngredients(List<String> requiredIngredientsList) {
        AtomicBoolean containsAnyRequiredIngredient = new AtomicBoolean(true);

        requiredIngredientsList.parallelStream().forEach(requiredIngredient -> {
            try {
                WebElement ingredientsWebElement = tlDriver.findElement(By.xpath("//div[@id='rcpinglist']"));
                String ingredients = ingredientsWebElement.getText();
                if (null != ingredients && null != requiredIngredient && !ingredients.toLowerCase().contains(requiredIngredient.toLowerCase())) {
                    System.out.println("Missing required ingredient: " + requiredIngredient);
                    containsAnyRequiredIngredient.set(false);
                }
            } catch (Exception e) {
                System.out.print("No Such Element " + e.getLocalizedMessage());
            }
        });
        return containsAnyRequiredIngredient.get();
    }
}